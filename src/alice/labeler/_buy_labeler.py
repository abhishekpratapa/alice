...
class BuyLabeler(LabelerTemplate):
    ...
    def identifier(self):
        return 'Buy_Sell_Probabilities'

    def process_data(self, bars):
        return_dict = self.__get_date(bars)
        sell_dict, buy_dict = self.__process_frequency(bars)
        
        for bd in buy_dict.keys():
            return_dict[bd] = buy_dict[bd]

        buy_arr = []
        for rd in return_dict.keys():
            buy_arr.append(return_dict[rd])
        buy_arr = self.__norm_array(buy_arr)

        buy_label = []
        for b in buy_arr:
            if b > 0.7:
                buy_label.append([0, 1])
            else:
                buy_label.append([1, 0])
        
        return np.array(buy_label)